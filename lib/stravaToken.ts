...
export async function resolveStravaToken(): Promise<ResolvedToken | null> {
  try { const token = await getValidAccessToken(); if (token) return { accessToken: token, source: "oauth_cookie" }; } catch {}
  ...
  if (_cache && now < _cache.expiresAt - BUFFER_SECS) return { accessToken: _cache.accessToken, source: "env_refresh" };
  try {
